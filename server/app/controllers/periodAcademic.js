...
exports.createPeriodAcademic = async(req, res) => {
    try {
        ...
        if (year < currentYear || year > currentYear + 1) {
          throw new Error('Solo se pueden crear períodos para el año actual o el año siguiente.');
        }
        ...
        throw new Error('La duración del período no está dentro del rango válido (entre 3.5 y 4 meses).');
        }
    ...
    const detailsPeriodData = {
      ...
      ADD_CANCELLATIONS_START_DATE: moment.utc(startDate).add(8, 'days').toDate(),
      ADD_CANCELLATIONS_END_DATE: moment.utc(startDate).add(14, 'days').toDate(),
      CLASS_START_DATE: moment.utc(startDate).add(8, 'days').toDate(),
      CLASS_END_DATE: moment.utc(startDate).add(88, 'days').toDate(),
      REGISTRATION_PAYMENT_START_DATE: moment.utc(startDate).subtract(5, 'days').toDate(),
      REGISTRATION_PAYMENT_END_DATE: moment.utc(startDate).add(14, 'days').toDate(),
      LABORATORIES_PAYMENT_START_DATE: moment.utc(startDate).add(15, 'days').toDate(),
      LABORATORIES_PAYMENT_END_DATE: moment.utc(startDate).add(21, 'days').toDate(),
      ...
      NOTES_UPLOAD_REGISTRATION_START_DATE: moment.utc(startDate).add(88, 'days').toDate(),
      NOTES_UPLOAD_REGISTRATION_END_DATE: moment.utc(startDate).add(89, 'days').toDate(),
    };
...
exports.deletePeriodAcademic = async (req, res) => {
  try {
    ...
    if (period.STATUS !== 'Por empezar') {
      throw new Error('Solo se pueden eliminar períodos con el STATUS "Por empezar".');
    }
    ...
    exports.getPeriodsByYear = async function(req, res) {
      try {
        ...
        // Verificar si el año es válido (puedes agregar más validaciones si es necesario)
        if (!year || isNaN(year)) {
